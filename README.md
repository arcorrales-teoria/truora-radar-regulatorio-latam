...
- **Un solo morado estandarizado: `indigo-600`.** Se usa para botones/accent, líneas decorativas y el fondo de las cards que SÍ siguen siendo oscuras a propósito (diagnóstico, countdown, panel de TestCta, burbuja/card de país del Hero) — esas cards ya eran oscuras antes del cambio de fondo de página y no se tocaron, siguen funcionando como acentos de color sobre el fondo aún más oscuro.
...
- **Todos los botones usan `variant="minimal"`** de `texture-button.tsx` (blanco/gris claro con degradado sutil) — pedido explícito de estandarización. Ese variant no fija color de texto propio (su clase base usa `text-muted-foreground`, un token de shadcn que este proyecto no define, así que no genera ninguna regla real): cualquier botón `minimal` que viva dentro de un contenedor con texto blanco heredado necesita `text-neutral-900` explícito en su `className`, si no el texto queda invisible sobre el botón claro. Ya aplicado en los botones dentro de las cards oscuras (`AssessmentWizard.tsx`).
- **Líneas decorativas** (`SectionLines.tsx`, `PageRails.tsx`): trazo **continuo** (no punteado). `PageRails` usa `indigo-400/50` (más claro que antes) para seguir siendo visible sobre el fondo azul medianoche, mucho más oscuro que el blanco anterior.
...
- **Excepciones documentadas, no tocar sin preguntar primero:** (1) los badges de color por país en `LawCountdownCard` (azul/verde/rosa, distinguen CO/CL/PE/MX, no es parte del eje morado/medianoche); (2) las conexiones animadas entre países de `LatamMap.tsx` (puntos punteados que fluyen) — reemplazan a propósito una línea de respaldo estática que se pidió quitar en una ronda anterior; convertirlas a sólido eliminaría esa animación de señal; (3) los colores del dot-map de `LatamMap.tsx` (índigo translúcido) no se recalibraron para el nuevo fondo oscuro — ya se leen bien como "constelación brillando", efecto deseable, no un bug.
